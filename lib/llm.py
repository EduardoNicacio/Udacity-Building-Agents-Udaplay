...
class LLM:
    def __init__(
        ...
        self.client = OpenAI(
            api_key=api_key, 
            base_url=("https://openai.vocareum.com/v1" if api_key.lower().startswith("voc") else None)
        )
    ...
    def invoke(self, 
        ...
        if response_format and content is None and getattr(message, "parsed", None) is not None:
